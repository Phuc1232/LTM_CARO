        ...
        private void CreateBoard()
        {
            for (int row = 0; row < ROWS; row++)
            {
                for (int col = 0; col < COLS; col++)
                {
                    ...
                    cell.FlatAppearance.BorderColor = Color.FromArgb(70, 210, 210);

                    cell.Tag = $"{row},{col}";
                    cell.BackColor = Color.FromArgb(128, 255, 255);
        ...
        public void SetCell(int row, int col, string text)
        {
            foreach (Control control in Controls)
            {
                if (control is Button btn && btn.Tag?.ToString() == $"{row},{col}")
                {
                    ...
                    {
                        _lastMoveButton.BackColor = Color.FromArgb(128, 255, 255);
                        _lastMoveButton.ForeColor = Color.Black;
